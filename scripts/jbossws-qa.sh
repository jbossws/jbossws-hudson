...
copyTestLogs() {
  ...
  for i in `find $STACK_DIR -name surefire-reports`; do egrep -h "skipped message"\|FIXME\|"Test skipped: " $i/*|sort -u|sed 's/    <skipped message=\"//'|sed 's/\"\/>//'|sed 's/Test skipped: //' >> $WORKSPACE/errata-$JBOSS_TARGET.txt; done;
}
